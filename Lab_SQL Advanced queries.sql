...
-- Step 1: list actors and the films they've acted on
select fa.actor_id, concat(a.first_name, ' ', a.last_name) as actor_name, fa.film_id
from sakila.film_actor fa
join sakila.actor a
on fa.actor_id = a.actor_id;

-- Step 2: Create two Common Table Expressions (CTE) with the previous querry and join them after
...
-- Step 1: List all the actor and count how many films they've been on
select actor_id, count(film_id) as film_count
from sakila.film_actor
group by actor_id;

-- Step 2: list actors and the films they've acted on
...
-- Step 3: Join the previous two querries
...
-- Step 4: Create a view with the previous querry
...
-- Step 5: Select the max_film_count per film
select film_id, max(film_count) as max_film_count from film_actor_film_count
group by film_id;

-- Step 5: Join the view with the step 5 querry in a Common Table Expressions (CTE):
